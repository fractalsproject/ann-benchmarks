...
def create_plot(all_data, raw, x_log, y_log, xn, yn, fn_out, linestyles,
    ...
    for algo in sorted(all_data.keys(), key=lambda x: x.lower()):
        ...
        color, faded, linestyle, marker = linestyles[algo[:10]]
    ...
    plt.gca().set_title("Faiss-ivf Performance on 54 Million Vectors at Different nlist Scores")
...
if __name__ == "__main__":
    ...
    unique_algorithms.add('nlist=0064')
    unique_algorithms.add("nlist=1024")
    unique_algorithms.add("nlist=8192")
    
    results = load_all_results(args.dataset, count, True, args.batch)
    linestyles = create_linestyles(sorted(unique_algorithms))
    runs = compute_metrics(np.array(dataset["distances"]),
                           results, args.x_axis, "qps", args.recompute)
    if not runs:
